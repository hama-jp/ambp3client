...
- **カート情報表示**: カート番号とドライバー名を表示（登録されている場合）
...
トランスポンダー選択時にカート番号や名前を表示するには、データベースの`karts`テーブルに登録します：

```sql
INSERT INTO karts (transponder_id, name, kart_number)
